...
def test_mcp_tool_sync_wrapper_generation():
    ...
    with (
        ...
        patch("marketior.config.extensions_config.ExtensionsConfig.from_file"),
        patch("marketior.mcp.tools.build_servers_config", return_value={"test-server": {}}),
        patch("marketior.mcp.tools.get_initial_oauth_headers", new_callable=AsyncMock, return_value={}),
    ):
...
def test_mcp_tool_sync_wrapper_exception_logging():
    ...
    with patch("marketior.tools.sync.logger.error") as mock_log_error:
