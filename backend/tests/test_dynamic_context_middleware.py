...
from marketior.agents.middlewares.dynamic_context_middleware import (
...
def test_injects_system_reminder_into_first_human_message():
    ...
    with mock.patch("marketior.agents.lead_agent.prompt._get_memory_context", return_value=""), mock.patch("marketior.agents.middlewares.dynamic_context_middleware.datetime") as mock_dt:
...
def test_memory_included_when_present():
    ...
    with (
        mock.patch(
            "marketior.agents.lead_agent.prompt._get_memory_context",
            return_value="<memory>\nUser prefers Python.\n</memory>",
        ),
        mock.patch("marketior.agents.middlewares.dynamic_context_middleware.datetime") as mock_dt,
    ):
        mock_dt.now.return_value.strftime.return_value = "2026-05-08, Friday"
        result = mw.before_agent(state, _fake_runtime())
...
def test_no_human_message_returns_none():
    ...
    with mock.patch("marketior.agents.lead_agent.prompt._get_memory_context", return_value=""):
        result = mw.before_agent(state, _fake_runtime())
    assert result is None
