...
def test_entrypoint_excludes_runtime_state_from_uvicorn_reload():
    ...
    assert ': "${MARKETIOR_HOME:=/app/backend/.deer-flow}"' in content
    # sandbox must be created too, not just .deer-flow (#3459 / #3454).
    assert 'mkdir -p "$MARKETIOR_HOME" /app/backend/.deer-flow /app/backend/sandbox' in content
    ...
    assert '--reload-exclude="$MARKETIOR_HOME"' in content
