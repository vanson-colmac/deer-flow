...
async def test_abefore_agent_does_not_block_event_loop() -> None:
    ...
    with (
        ...
        mock.patch("marketior.agents.lead_agent.prompt._get_memory_context", return_value=""),
    ):
...
async def test_abefore_agent_returns_none_on_timeout() -> None:
    ...
    with (
        ...
        mock.patch(
            "marketior.agents.middlewares.dynamic_context_middleware._INJECT_TIMEOUT_SECONDS",
            0.1,
        ),
    ):
        state = {"messages": [HumanMessage(content="Hello", id="msg-1")]}
