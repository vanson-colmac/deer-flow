...
class TestEnsureAgent:
    def test_creates_agent(self, client):
        ...
        with (
            ...
            patch("marketior.client.build_middlewares", return_value=[]) as mock_build_middlewares,
    ...
    def test_injects_custom_middlewares(self, client):
        ...
        with (
            patch("marketior.client.create_chat_model"),
            patch("marketior.client.create_agent", return_value=mock_agent) as mock_create_agent,
            patch("marketior.client.build_middlewares", side_effect=fake_build_middlewares),
            patch("marketior.client.apply_prompt_template", return_value="prompt"),
            patch.object(client, "_get_tools", return_value=[]),
            patch("marketior.runtime.checkpointer.get_checkpointer", return_value=MagicMock()),
        ):
