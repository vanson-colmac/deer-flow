...
from marketior.config import skills_config as skills_config_module
...
from marketior.config.runtime_paths import project_root
...
def _clear_path_env(monkeypatch):
    for name in (
        ...
        "MARKETIOR_EXTENSIONS_CONFIG_PATH",
        "MARKETIOR_HOME",
        "MARKETIOR_PROJECT_ROOT",
        "MARKETIOR_SKILLS_PATH",
    ):
...
def test_default_runtime_paths_resolve_from_current_project(tmp_path: Path, monkeypatch):
    ...
    (tmp_path / "config.yaml").write_text(
        yaml.safe_dump({"sandbox": {"use": "marketior.sandbox.local:LocalSandboxProvider"}}),
        encoding="utf-8",
    )
    ...
    assert Paths().base_dir == tmp_path / ".marketior"
...
def test_marketior_project_root_overrides_current_directory(tmp_path: Path, monkeypatch):
    _clear_path_env(monkeypatch)
    ...
    monkeypatch.setenv("MARKETIOR_PROJECT_ROOT", str(project_root))
    ...
    assert Paths().base_dir == project_root / ".marketior"
    assert SkillsConfig(path="custom-skills").get_skills_path() == project_root / "custom-skills"


def test_marketior_skills_path_overrides_project_default(tmp_path: Path, monkeypatch):
    _clear_path_env(monkeypatch)
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("MARKETIOR_SKILLS_PATH", "team-skills")
...
def test_marketior_project_root_must_exist(tmp_path: Path, monkeypatch):
    _clear_path_env(monkeypatch)
    missing_root = tmp_path / "missing"
    monkeypatch.setenv("MARKETIOR_PROJECT_ROOT", str(missing_root))

    with pytest.raises(ValueError, match="does not exist"):
        project_root()


def test_marketior_project_root_must_be_directory(tmp_path: Path, monkeypatch):
    _clear_path_env(monkeypatch)
    project_root_file = tmp_path / "project-root"
    project_root_file.write_text("", encoding="utf-8")
    monkeypatch.setenv("MARKETIOR_PROJECT_ROOT", str(project_root_file))
...
def test_app_config_falls_back_to_legacy_when_project_root_lacks_config(tmp_path: Path, monkeypatch):
    """When MARKETIOR_PROJECT_ROOT is unset and cwd has no config.yaml, the
...
def test_skills_config_falls_back_to_legacy_when_project_root_lacks_skills(tmp_path: Path, monkeypatch):
    """When MARKETIOR_PROJECT_ROOT is unset and cwd has no `skills/`, the legacy
...
def test_extensions_config_falls_back_to_legacy_when_project_root_lacks_file(tmp_path: Path, monkeypatch):
    ...
    fake_paths_module_file = fake_backend / "packages" / "harness" / "marketior" / "config" / "extensions_config.py"
