...
from marketior.sandbox.exceptions import SandboxError
...
def test_mask_local_paths_in_output_hides_skills_host_paths() -> None:
    ...
    with (
        ...
        patch("marketior.sandbox.tools._get_skills_host_path", return_value="/home/user/deer-flow/skills"),
    ):
...
def test_write_file_tool_bounds_large_oserror_and_masks_local_paths(monkeypatch) -> None:
    ...
    monkeypatch.setattr("marketior.sandbox.tools.ensure_sandbox_initialized", lambda runtime: sandbox)
    monkeypatch.setattr("marketior.sandbox.tools.ensure_thread_directories_exist", lambda runtime: None)
    monkeypatch.setattr("marketior.sandbox.tools.is_local_sandbox", lambda runtime: True)
    monkeypatch.setattr("marketior.sandbox.tools.get_thread_data", lambda runtime: _THREAD_DATA)
    monkeypatch.setattr("marketior.sandbox.tools.validate_local_tool_path", lambda path, thread_data: None)
    monkeypatch.setattr(
        "marketior.sandbox.tools._resolve_and_validate_user_data_path",
...
def test_write_file_tool_handles_sandbox_init_failure(monkeypatch) -> None:
    ...
    runtime = SimpleNamespace(state={}, context={"thread_id": "thread-1"}, config={})
    monkeypatch.setattr("marketior.sandbox.tools.ensure_sandbox_initialized", raise_sandbox_error)
    monkeypatch.setattr("marketior.sandbox.tools.is_local_sandbox", lambda runtime: False)
