...
from marketior.config.runtime_paths import runtime_home
...
def _default_local_base_dir() -> Path:
    """Return the caller project's writable Marketior state directory."""
    return runtime_home()
...
def _join_host_path(base: str, *parts: str) -> str:
    ...
    path form (for example ``C:\\repo\\backend\\.marketior``).  Using
...
class Paths:
    """
    Centralized path configuration for Marketior application data.
    ...
    BaseDir resolution (in priority order):
        ...
        2. MARKETIOR_HOME environment variable
        3. Caller project fallback: `{project_root}/.marketior`
    """
    ...
    def host_base_dir(self) -> Path:
        ...
        Set MARKETIOR_HOST_BASE_DIR to the host-side path that corresponds to this
        ...
        """
        if env := os.getenv("MARKETIOR_HOST_BASE_DIR"):
            return Path(env)
    ...
    def base_dir(self) -> Path:
        ...
        if env_home := os.getenv("MARKETIOR_HOME"):
