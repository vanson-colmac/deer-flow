...
from marketior.tracing import inject_langfuse_metadata
...
async def run_agent(
    ...
    *,
    ...
    try:
        ...
        # Shared helper with ``MarketiorClient.stream`` so both entry points stay
        ...
        inject_langfuse_metadata(
            ...
            environment=os.environ.get("MARKETIOR_ENV") or os.environ.get("ENVIRONMENT"),
        )
...
def _try_extract_from_message(obj: Any) -> str | None:
    ...
    if isinstance(additional_kwargs, dict) and additional_kwargs.get("marketior_error_fallback"):
    ...
    if isinstance(obj, dict):
        ...
        if isinstance(nested_kwargs, dict) and nested_kwargs.get("marketior_error_fallback"):
