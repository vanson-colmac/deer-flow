...
def _build_runtime_middlewares(
    *,
    ...
    from marketior.agents.middlewares.llm_error_handling_middleware import LLMErrorHandlingMiddleware
    from marketior.agents.middlewares.thread_data_middleware import ThreadDataMiddleware
    from marketior.agents.middlewares.tool_output_budget_middleware import ToolOutputBudgetMiddleware
        ...
        if "framework" not in provider_kwargs:
            try:
                ...
                if "framework" in sig.parameters or any(p.kind == inspect.Parameter.VAR_KEYWORD for p in sig.parameters.values()):
                    provider_kwargs["framework"] = "marketior"
    ...
    from marketior.agents.middlewares.sandbox_audit_middleware import SandboxAuditMiddleware
