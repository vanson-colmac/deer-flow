...
| | `langchain.create_agent` | `make_lead_agent` | `MarketiorClient`（增强后） |
|---|---|---|---|
...
| 适合谁 | 写 LangChain 的人 | 内部使用 | **所有 Marketior 用户** |
...
```
    ┌──────────────────────┐
    │   MarketiorClient     │  ← 唯一公开 API（chat/stream + 管理）
    └──────────┬───────────┘
    ┌──────────▼───────────┐
...
`MarketiorClient` 是唯一公开 API。`create_marketior_agent` 和 `make_lead_agent` 都是内部实现。

用户通过 `MarketiorClient` 三个参数控制行为：
...
- **向前兼容** — 现有 `MarketiorClient()` 无参构造行为不变
- **harness 边界合规** — 不 import `app.*`（`test_harness_boundary.py` 强制）

## 3. API 设计

### 3.1 `MarketiorClient` — 唯一公开 API
...
# 用法 2：只改参数，不换实现
client = MarketiorClient(config={"memory": {"max_facts": 50}})

# 用法 3：替换 middleware 实现
client = MarketiorClient(features=RuntimeFeatures(auto_title=MyTitleMiddleware()))

# 用法 4：添加自定义 middleware
client = MarketiorClient(extra_middleware=[MyAuditMiddleware()])

# 用法 5：纯 SDK（无 config.yaml）
client = MarketiorClient(config={
...
client = MarketiorClient(features=RuntimeFeatures(memory=MyMemoryMiddleware()))
...
Phase 1（当前 PR #1203）:
  ...
  ✓ 不改 MarketiorClient 和 make_lead_agent
  ✗ middleware 内部仍读 config（已知限制）

Phase 2（#1380）:
  - MarketiorClient 构造函数增加可选参数（model, tools, features, system_prompt）
...
| 问题 | 决议 | 理由 |
|------|------|------|
| 公开 API | `MarketiorClient` 唯一入口 | 自顶向下，先改现有 API 再抽底层 |
