...
本文档描述 Marketior 当前内置认证模块的设计，而不是历史 RFC。它覆盖浏览器登录、API 认证、CSRF、用户隔离、首次初始化、密码重置、内部调用和升级迁移。

## 设计目标

认证模块的核心目标是把 Marketior 从“本地单用户工具”提升为“可多用户部署的 agent runtime”，并让用户身份贯穿 HTTP API、LangGraph-compatible runtime、文件系统、memory、自定义 agent 和反馈数据。
...
- `request.state.user`
- `request.state.auth`
- `marketior.runtime.user_context` 的 `ContextVar`
...
- `token_version += 1`。
- 设置 `needs_setup=true`。
- 写入 `.marketior/admin_initial_credentials.txt`，权限 `0600`。
...
Marketior 使用 Double Submit Cookie：
...
- 请求带 `X-Marketior-Internal-Token`。
- 同时带匹配的 CSRF cookie/header。
- 服务端识别为内部用户，`id="default"`、`system_role="internal"`。

这意味着 channel 产生的数据默认进入 `default` 用户桶。这个选择适合“平台级 bot 身份”，但不是“每个 IM 用户单独隔离”。如果后续要做到外部 IM 用户隔离，需要把外部 platform user 映射到 Marketior user，并让 channel manager 设置对应的 scoped identity。
...
| IM 用户隔离 | channel 使用 `default` 内部用户 | 建立外部用户到 Marketior user 的映射 |
...
| `marketior/runtime/user_context.py` | 当前用户 ContextVar 与 `AUTO` sentinel |
| `marketior/persistence/thread_meta/` | thread metadata owner filter |
| `marketior/config/paths.py` | per-user filesystem layout |
| `marketior/agents/middlewares/thread_data_middleware.py` | run 时解析用户线程目录 |
| `marketior/agents/memory/storage.py` | per-user memory storage |
| `marketior/config/agents_config.py` | per-user custom agents |
| `app/channels/manager.py` | IM channel 内部认证调用 |
| `scripts/migrate_user_isolation.py` | legacy 数据迁移到 per-user layout |
| `.marketior/data/marketior.db` | 统一 SQLite 数据库，包含 users / threads_meta / runs / feedback 等表 |
| `.marketior/users/{user_id}/agents/{agent_name}/` | 用户自定义 agent 配置、SOUL 和 agent memory |
| `.marketior/admin_initial_credentials.txt` | `reset_admin` 生成的新凭据文件（0600，读完应删除） |
