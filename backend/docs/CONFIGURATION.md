...
This guide explains how to configure Marketior for your environment.
...
- On macOS, Marketior does not probe Keychain automatically. Use `scripts/export_claude_code_oauth.py` to export Claude Code auth explicitly when needed
...
Marketior supports multiple sandbox execution modes. Configure your preferred mode in `config.yaml`:
...
When using Docker development (`make docker-start`), Marketior starts the `provisioner` service only if this provisioner mode is configured. In local or plain Docker sandbox modes, `provisioner` is skipped.
...
`allow_host_bash` is intentionally `false` by default. Marketior's local sandbox is a host-side convenience mode, not a secure shell isolation boundary. If you need `bash`, prefer `AioSandboxProvider`. Only set `allow_host_bash: true` for fully trusted single-user local workflows.
...
When you configure `sandbox.mounts`, Marketior exposes those `container_path` values in the agent prompt so the agent can discover and operate on mounted directories directly instead of assuming everything must live under `/mnt/user-data`.

For bare-metal Docker sandbox runs that use localhost, Marketior binds the sandbox HTTP port to `127.0.0.1` by default so it is not exposed on every host interface. Docker-outside-of-Docker deployments that connect through `host.docker.internal` keep the broad legacy bind for compatibility. Set `MARKETIOR_SANDBOX_BIND_HOST` explicitly if your deployment needs a different bind address.
...
**How Skills Work**:
- Skills are stored in `marketior/skills/{public,custom}/`
...
2. Restart the Marketior service to apply changes

## Environment Variables

Marketior supports environment variable substitution using the `$` prefix:
...
- `MARKETIOR_HOME` - Runtime state directory (defaults to `.marketior` under the project root)
...
The configuration file should be placed in the **project root directory** (`marketior/config.yaml`). Set `MARKETIOR_PROJECT_ROOT` when the process may start from another working directory, or set `MARKETIOR_CONFIG_PATH` to point at a specific file.

## Configuration Priority

Marketior searches for configuration in this order:
...
- Ensure `config.yaml` exists in the **project root** directory (`marketior/config.yaml`)
...
### "Skills not loading"
- Check that `marketior/skills/` directory exists
