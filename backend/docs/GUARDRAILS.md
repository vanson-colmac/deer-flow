...
The simplest option. Ships with Marketior.AI. Block or allow tools by name. No external packages, no passport, no network.
...
guardrails:
  enabled: true
  provider:
    use: marketior.guardrails.builtin:AllowlistProvider
    config:
...
aport setup --framework marketior
```

This creates:
- `~/.aport/marketior/config.yaml` -- evaluator config (local or API mode)
- `~/.aport/marketior/aport/passport.json` -- OAP passport with capabilities and limits
...
Any provider that accepts `framework` as a kwarg and implements `evaluate`/`aevaluate` works. The OAP standard defines the passport format and decision codes; Marketior.AI doesn't care which provider reads them.
...
A custom OAP provider can implement any evaluation strategy -- the Marketior.AI middleware doesn't care how the provider reaches its decision.

**Try it:**
1. Install and set up as above
2. Start Marketior.AI and ask: "Create a file called test.txt with content hello"
...
class MyGuardrailProvider:
    ...
    def evaluate(self, request):
        from marketior.guardrails.provider import GuardrailDecision, GuardrailReason
...
2. Add the config
3. Start Marketior.AI and ask: "Use bash to delete test.txt"
...
Marketior.AI loads providers via `resolve_variable()` -- the same mechanism used for models, tools, and sandbox providers. The `use:` field is a Python class path: `package.module:ClassName`.

The provider is instantiated with `**config` kwargs if `config:` is set, plus `framework="marketior"` is always injected. Accept `**kwargs` to stay forward-compatible:

```python
class YourProvider:
    def __init__(self, framework: str = "generic", **kwargs):
        # framework="marketior" tells you which config dir to use
        ...
```
...
```
packages/harness/marketior/guardrails/
...
packages/harness/marketior/config/
    guardrails_config.py     # GuardrailsConfig Pydantic model + singleton

packages/harness/marketior/agents/middlewares/
