...
The simplest option. Ships with Marketior. Block or allow tools by name. No external packages, no passport, no network.
...
Any provider that accepts `framework` as a kwarg and implements `evaluate`/`aevaluate` works. The OAP standard defines the passport format and decision codes; Marketior doesn't care which provider reads them.
...
A custom OAP provider can implement any evaluation strategy -- the Marketior middleware doesn't care how the provider reaches its decision.

**Try it:**
1. Install and set up as above
2. Start Marketior and ask: "Create a file called test.txt with content hello"
...
2. Add the config
3. Start Marketior and ask: "Use bash to delete test.txt"
...
Marketior loads providers via `resolve_variable()` -- the same mechanism used for models, tools, and sandbox providers. The `use:` field is a Python class path: `package.module:ClassName`.
