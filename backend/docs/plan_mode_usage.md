...
This document describes how to enable and use the Plan Mode feature with TodoList middleware in Marketior.AI 2.0.
...
from marketior.agents.lead_agent.agent import make_lead_agent
...
### Agent Module
- **Location**: `packages/harness/marketior/agents/lead_agent/agent.py`
...
Marketior.AI uses custom `system_prompt` and `tool_description` for the TodoListMiddleware that match the overall Marketior.AI prompt style:

### System Prompt Features
- Uses XML tags (`<todo_list_system>`) for structure consistency with Marketior.AI's main prompt
...
The custom prompts are defined in `_create_todo_list_middleware()` in `/Users/hetao/workspace/deer-flow/backend/packages/harness/marketior/agents/lead_agent/agent.py:57`.

## Notes

- TodoList middleware uses LangChain's built-in `TodoListMiddleware` with **custom Marketior.AI-style prompts**
...
- Custom prompts emphasize the same principles as Marketior.AI's main system prompt (clarity, action-oriented, critical rules)
