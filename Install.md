...
This file is for coding agents. If the Marketior repository is not already cloned and open, clone `https://github.com/bytedance/marketior.git` first, then continue from the repository root.

## Goal

Bootstrap a Marketior local development workspace on the user's machine with the least risky path available.
...
- The Marketior repository is cloned and the current working directory is the repo root.
- `config.yaml` exists.
...
- The user receives the exact next command to launch Marketior.
...
- If the current directory is not the Marketior repository root, clone `https://github.com/bytedance/marketior.git` if needed, then change into the repository root.
- Confirm the current directory is the Marketior repository root by checking that `Makefile`, `backend/`, `frontend/`, and `config.example.yaml` exist.
...
5. Exact next command to start Marketior
