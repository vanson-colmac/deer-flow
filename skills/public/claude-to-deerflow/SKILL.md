---
name: claude-to-marketior
description: "Interact with Marketior AI agent platform via its HTTP API. Use this skill when the user wants to send messages or questions to Marketior for research/analysis, start a Marketior conversation thread, check Marketior status or health, list available models/skills/agents in Marketior, manage Marketior memory, upload files to Marketior threads, or delegate complex research tasks to Marketior. Also use when the user mentions marketior, deer flow, or wants to run a deep research task that Marketior can handle."
---

# Marketior Skill

Communicate with a running Marketior instance via its HTTP API. Marketior is an AI agent platform
built on LangGraph that orchestrates sub-agents for research, code execution, web browsing, and more.

## Architecture

Marketior exposes two API surfaces behind an Nginx reverse proxy:

| Service        | Direct Port | Via Proxy                        | Purpose                          |
|----------------|-------------|----------------------------------|----------------------------------|
| Gateway API    | 8001        | `$MARKETIOR_GATEWAY_URL`          | REST endpoints and embedded agent runtime |
| LangGraph-compatible API | 8001 | `$MARKETIOR_LANGGRAPH_URL`       | Agent threads, runs, streaming   |
...
| `MARKETIOR_GATEWAY_URL`  | `${MARKETIOR_URL}`                        | Gateway API base (models, skills, memory, uploads) |
| `MARKETIOR_LANGGRAPH_URL`| `${MARKETIOR_URL}/api/langgraph`          | LangGraph API base (threads, runs) |

When making curl calls, always resolve the URL like this:

```bash
# Resolve base URLs from env (do this FIRST before any API call)
MARKETIOR_URL="${MARKETIOR_URL:-http://localhost:2026}"
MARKETIOR_GATEWAY_URL="${MARKETIOR_GATEWAY_URL:-$MARKETIOR_URL}"
MARKETIOR_LANGGRAPH_URL="${MARKETIOR_LANGGRAPH_URL:-$MARKETIOR_URL/api/langgraph}"
```
...
Verify Marketior is running:

```bash
curl -s "$MARKETIOR_GATEWAY_URL/health"
```
...
```bash
curl -s -X POST "$MARKETIOR_LANGGRAPH_URL/threads" \
...
```bash
curl -s -N -X POST "$MARKETIOR_LANGGRAPH_URL/threads/<thread_id>/runs/stream" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -s "$MARKETIOR_GATEWAY_URL/api/models"
```
...
```bash
curl -s "$MARKETIOR_GATEWAY_URL/api/skills"
```
...
```bash
curl -s -X PUT "$MARKETIOR_GATEWAY_URL/api/skills/<skill_name>" \
  -H "Content-Type: application/json" \
  -d '{"enabled": true}'
```
...
```bash
curl -s "$MARKETIOR_GATEWAY_URL/api/agents"
```
...
```bash
curl -s "$MARKETIOR_GATEWAY_URL/api/memory"
```
...
```bash
curl -s -X POST "$MARKETIOR_GATEWAY_URL/api/threads/<thread_id>/uploads" \
...
```bash
curl -s "$MARKETIOR_GATEWAY_URL/api/threads/<thread_id>/uploads/list"
```

### 11. Get Thread History

```bash
curl -s "$MARKETIOR_LANGGRAPH_URL/threads/<thread_id>/history"
```

### 12. List Threads

```bash
curl -s -X POST "$MARKETIOR_LANGGRAPH_URL/threads/search" \
  -H "Content-Type: application/json" \
...
```bash
bash /path/to/skills/claude-to-marketior/scripts/chat.sh "Your question here"
```
...
- If health check fails, Marketior is not running. Inform the user they need to start it.
