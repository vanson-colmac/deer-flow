...
export function getGatewayConfig(): GatewayConfig {
  ...
  const rawUrl = process.env.MARKETIOR_INTERNAL_GATEWAY_BASE_URL?.trim() || process.env.DEER_FLOW_INTERNAL_GATEWAY_BASE_URL?.trim();
  const internalGatewayUrl =
    rawUrl && rawUrl.length > 0
      ? rawUrl.replace(/\/+$/, "")
      : "http://127.0.0.1:8001";

  const rawOrigins = process.env.MARKETIOR_TRUSTED_ORIGINS?.trim() || process.env.DEER_FLOW_TRUSTED_ORIGINS?.trim();
