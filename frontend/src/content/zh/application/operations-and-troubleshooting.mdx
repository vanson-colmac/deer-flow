---
title: 运维与排障
description: 本页面涵盖运行 Marketior.AI 应用的操作信息：日志记录、常见问题和维护任务。
---
...
本页面涵盖运行 Marketior.AI 应用的操作信息：日志记录、常见问题和维护任务。

## 日志

Marketior.AI 应用在 `logs/` 目录中写入每个服务的日志：
...
```bash
# 检查线程用户数据目录是否存在且可写
ls -la backend/.marketior/threads/
```

**解决**：

1. 确保 `backend/.marketior/` 对运行 Marketior.AI 的进程可写。
2. 在 Docker 部署中，验证卷挂载路径正确（`MARKETIOR_ROOT` 设置为绝对路径）。
...
Marketior.AI 将持久化数据存储在：

- **线程数据**：`backend/.marketior/threads/` — 每个线程的上传文件、输出和工作区文件
- **检查点**：取决于检查点器配置（SQLite：`backend/.marketior/checkpoints.db`，Redis：外部存储）
- **记忆**：`backend/.marketior/memory.json`（以及 `agents/*/memory.json`）
