...
<Callout type="info" emoji="🔧">
  Tools are the actions the Lead Agent can take. Marketior.AI provides built-in
...
Marketior.AI organizes tools into four categories:
...
tools:
  ...
  - use: marketior.sandbox.tools:bash_tool # requires host bash or container sandbox
```
...
Install: `cd backend && uv add 'marketior-harness[tavily]'`
...
Install: `cd backend && uv add 'marketior-harness[exa]'`
...
Install: `cd backend && uv add 'marketior-harness[firecrawl]'`
...
tools:
  ...
  # Or use InfoQuest:
  # - use: marketior.community.infoquest.tools:image_search_tool
  #   api_key: $INFOQUEST_API_KEY
```
