---
title: Memory
description: Memory is a runtime feature of the Marketior.AI Harness. It is not a simple conversation log — it is a structured store of facts and context summaries that persist across separate sessions and inform the agent's behavior in future conversations.
---
...
<Callout type="info" emoji="💾">
  Memory lets Marketior.AI carry useful information across sessions. The agent
...
Memory is a runtime feature of the Marketior.AI Harness. It is not a simple conversation log — it is a structured store of facts and context summaries that persist across separate sessions and inform the agent's behavior in future conversations.
...
memory:
  ...
  # Default: {base_dir}/memory.json  (resolves to backend/.marketior/memory.json)
  ...
  storage_class: marketior.agents.memory.storage.FileMemoryStorage
...
Marketior.AI supports two levels of memory:
...
- Base directory: `backend/.marketior/`
- Global memory: `backend/.marketior/memory.json`
- Per-agent memory: `backend/.marketior/agents/{agent_name}/memory.json`
