---
title: Configuration
description: Marketior.AI App is configured through two files and a set of environment variables. This page covers the application-level configuration that most operators need to set up before deploying.
---
...
Marketior.AI App is configured through two files and a set of environment variables. This page covers the application-level configuration that most operators need to set up before deploying.
...
Install Ollama provider: `cd backend && uv add 'marketior-harness[ollama]'`
...
Install: `cd backend && uv add 'marketior-harness[aio-sandbox]'`
...
tools:
  ...
  - use: marketior.community.ddg_search.tools:web_search_tool # default, no key required
  ...
  - use: marketior.sandbox.tools:str_replace_tool
  - use: marketior.sandbox.tools:bash_tool
```

### Thread state persistence (checkpointer)

By default, Marketior.AI uses an SQLite checkpointer for thread state persistence:

```yaml
checkpointer:
  type: sqlite
  connection_string: checkpoints.db # stored in backend/.marketior/
```
...
checkpointer:
  type: postgres
  connection_string: postgresql://user:password@localhost:5432/marketior
```
