...
description: "This guide covers all supported deployment methods for Marketior.AI App: local development, Docker Compose, and production with Kubernetes-managed sandboxes."
---
...
This guide covers all supported deployment methods for Marketior.AI App: local development, Docker Compose, and production with Kubernetes-managed sandboxes.

## Local development deployment

The local workflow is the fastest way to run Marketior.AI. All services run as native processes on your machine.
...
```bash
# Set the absolute path to your marketior repo root
export MARKETIOR_ROOT=/path/to/marketior
...
# .env
OPENAI_API_KEY=sk-...
MARKETIOR_ROOT=/absolute/path/to/marketior
...
Thread data is stored in `backend/.marketior/threads/`. In Docker deployments, this directory is bind-mounted into the gateway container.

To avoid data loss when containers are recreated:

1. Set `MARKETIOR_ROOT` to the absolute repo root path (or a stable host path).
...
USERDATA_PVC_NAME=marketior-userdata-pvc
SKILLS_PVC_NAME=marketior-skills-pvc
```
...
Marketior.AI App uses [Better Auth](https://www.better-auth.com/) for session management. In production:
