...
In Docker deployments, this path is bind-mounted from the host so that thread data persists across container restarts. Set `MARKETIOR_ROOT` to the absolute path of your deer-flow repository root to ensure the correct host path is used.
