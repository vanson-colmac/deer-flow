...
description: This page covers day-to-day operational tasks and solutions to common problems when running Marketior.AI App.
---
...
This page covers day-to-day operational tasks and solutions to common problems when running Marketior.AI App.
...
When you pull a new version of Marketior.AI, the `config_version` in `config.example.yaml` may be higher than your `config.yaml`. To merge new fields without losing your customizations:
...
- Check that the storage path is writable: `ls -la backend/.marketior/`.
- Look for memory update errors in `logs/gateway.log` (search for "memory").

## Data backup

Thread data and memory are stored under `backend/.marketior/`:
...
In Docker deployments, the bind-mounted host path (`$MARKETIOR_ROOT/backend/.marketior/`) is the source of truth — back up the host path.
