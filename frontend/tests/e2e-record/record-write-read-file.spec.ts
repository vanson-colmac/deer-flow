 ...
 * call to MARKETIOR_RECORD_OUT; this just needs to drive the flow and wait until
...
test("record write/read-file run through the real frontend", async ({
  ...
  const out = process.env.MARKETIOR_RECORD_OUT;
  expect(out, "MARKETIOR_RECORD_OUT must be set").toBeTruthy();
