...
if [ -n "$port_2026_usage" ]; then
    ...
    while IFS= read -r pid; do
        ...
        case "$process_command" in
            *[Dd]eer[Ff]low*|*[Dd]eerflow*|*[Nn]ginx*marketior*|*marketior/*[Nn]ginx*)
                marketior_process_found=1
                ;;
        esac
    done <<EOF
...
EOF
    ...
    if [ "$marketior_process_found" -eq 1 ]; then
        echo "✓ Port 2026 is occupied by Marketior"
    else
        echo "✗ Port 2026 must be free before starting Marketior"
        exit 1
    fi
else
